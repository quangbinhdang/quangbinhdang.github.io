...
function process_race(info) {
    ...
    document.getElementById('track_name').innerHTML = "Track name: " + race.trackName + " " + race.RaceNumber;
    document.getElementById('track_distance').innerHTML = "Track distance: " + race.Distance;
    document.getElementById('track_condition').innerHTML = "Track condition: " + race.Meeting.TrackCondition;
  ...
  var totalPrizePool = 0;
   ...
   race.Runners.forEach((runner) => {
        ...
        if (status != "Scratched") {
            ...
            var trackPerformanceWinPercentage = Math.round((runner.stats.trackPerformance.wins / runner.stats.trackPerformance.starts) * 100);
            var trackPerformancePlacePercentage = Math.round(((runner.stats.trackPerformance.wins + runner.stats.trackPerformance.seconds) / runner.stats.trackPerformance.starts) * 100);

            var distancePerformanceWinPercentage = Math.round((runner.stats.distancePerformance.wins / runner.stats.distancePerformance.starts) * 100);
            var distancePerformancePlacePercentage = Math.round(((runner.stats.distancePerformance.wins + runner.stats.distancePerformance.seconds) / runner.stats.distancePerformance.starts) * 100);

            var trackDistanceWinPercentage = Math.round((runner.stats.trackAndDistancePerformance.wins / runner.stats.trackAndDistancePerformance.starts) * 100);
            var trackDistancePlacePercentage = Math.round(((runner.stats.trackAndDistancePerformance.wins + runner.stats.trackAndDistancePerformance.seconds) / runner.stats.trackAndDistancePerformance.starts) * 100);
            ...
            var averagePrizeMoney = Math.round(runner.PrizeMoneyWon / runner.stats.allPerformance.starts);
            ...
            var formBonusObject = getFormBonus(race, runner, pointBreakdownComment, formCell);
            ...
            pointBreakdownComment += "-Track perfomance points " + trackBonus + " <br/>";
            ...
            pointBreakdownComment += "-Distance perfomance points " + distanceBonus + " <br/>";
            ...
            pointBreakdownComment += "-T/D perfomance points " + distanceBonus + " <br/>";

            if (barrier < 5) {
                speedBarrierBonus = runner.SpeedMap.Speed - runner.Barrier;
                pointBreakdownComment += "-Speed/Barrier points " + speedBarrierBonus + " <br/>";
            }


            if (race.Distance <= 1100) {
                speedDistanceBonus = runner.SpeedMap.Speed;
                pointBreakdownComment += "-Speed/Distance points " + speedDistanceBonus + " <br/>";
            }
            ...
            if (runner.Age >= 4 < 7) {
                ...
                ageBonus -= 3;
                pointBreakdownComment += "-Age less than 3 or more than 7, points " + ageBonus + " <br/>";
            }
            logger("jockeyBonus: " + jockeyBonus + " || speedBarrierBonus: " + speedBarrierBonus + " || speedDistanceBonus: " + speedDistanceBonus + " || distanceBonus: " + distanceBonus + " || trackDistanceBonus: " + trackDistanceBonus + " || formBonus: " + formBonus);
            var bonus = jockeyBonus + speedBarrierBonus + speedDistanceBonus + distanceBonus + trackBonus + trackDistanceBonus + conditionBonus + formBonus + ageBonus;
    ...
    averagePrizeArray.forEach(function(prize, prizeIndex) {
        availableRunners.forEach(function(runner, runnerIndex) {
            ...
            if (averagePrizeMoney == prize) {
                var pointsForPrize = Math.round((averagePrizeMoney / totalPrizePool) * totalPointsFromPrizePool);
                ...
                pointBreakdownComentArray[runnerIndex] += "-Average Prize, points " + pointsForPrize + " <br/>";
                pointBreakdownComentArray[runnerIndex] += "-Total points =  " + finalPoints + " <br/>";
                logger("comment = " + pointBreakdownComentArray[runnerIndex]);
                commentCellArray[runnerIndex].innerHTML = pointBreakdownComentArray[runnerIndex];
            }
        });
    });
}
...
function getFormBonus(race, runner, pointBreakdownComment, formCell){
  ...
  runner.PreviousForm.forEach(form => {
      if (form.Finish <= 3) {
          ...
          } else if (runner.Barrier == (form.Barrier + 1) || runner.Barrier == (form.Barrier - 1)) {
              pointBreakdownComment += "-Barrier +-1 points +2<br/>";
              formBonus += 2;
          } else if (runner.Barrier == (form.Barrier + 2) || runner.Barrier == (form.Barrier - 2)) {
      ...
      if (form.Finish <= 3) {
          ...
          if (form.TrackCondition == race.Meeting.TrackCondition.charAt(0)) {
              conditionBonus = 5;
              pointBreakdownComment += "-Condition points " + conditionBonus + " <br/>";
          }
          ...
          if (form.TimeRan != "N/A") {
              ...
              var weightDifference = Number(runner.Weight.Total) - (Number(form.BodyWeight) + Number(form.WeightCarried));
              logger("form.BodyWeight = " + form.BodyWeight + "form.WeightCarried = " + form.WeightCarried + "| weight = " + runner.Weight.Total + " | weightDifference = " + weightDifference + " | ADDED = " + Number(weightDifference * 50));
              ...
              if (race.Distance == 1400) {
                  if (form.TrackCondition.toLowerCase() == "g" && race.Meeting.TrackCondition.charAt(0).toLowerCase() == "h") {
                      miliToWin += 400;
                  } else if (form.TrackCondition.toLowerCase() == "h" && race.Meeting.TrackCondition.charAt(0).toLowerCase() == "g") {
                      miliToWin -= 400;
                  }
              }
              ...
              logger("minute = " + timeArray[0] + " second = " + second + " mili = " + remainingMiliToWin);
          }
          var formInfo = "";
          if (form.Distance == race.Distance) {
              formInfo += "-<b>Distance: " + form.Distance + "</b>";
          } else {
              formInfo += "-Distance: " + form.Distance;
          }
          if (form.TrackCondition == race.Meeting.TrackCondition.charAt(0)) {
          ...
          formInfo += "<br/>-Finish: " + finish + "/" + form.NumberOfRunners + "<br/>-Time: " + timeRan + "</br>-In run: " + form.InRun + "</br>-Expected Time: " + expectedWinTime + "<br/>-Margin: " + margin + "<br/>----------------<br/>";
          formCell.innerHTML += formInfo;
      }
      pointBreakdownComment += "------------------ <br/>";
      returnObject = {"formBonus" : formBonus, "pointBreakdownComment" : pointBreakdownComment};
  });
  return returnObject;
}
...
function getJockeyPoints(jockey, state) {
    logger("Jock = " + jockey.toLowerCase() + " | State = " + state.toLowerCase())
