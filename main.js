  ...
  horses.forEach((horse) => {
    ...
    if (status != "Scratched") {
      ...
      var trackPerformanceWinPercentage = Math.round(
        (horse.stats.trackPerformance.wins /
          horse.stats.trackPerformance.starts) *
          100
      );
      var trackPerformancePlacePercentage = Math.round(
        ((horse.stats.trackPerformance.wins +
          horse.stats.trackPerformance.seconds) /
          horse.stats.trackPerformance.starts) *
          100
      );

      var distancePerformanceWinPercentage = Math.round(
        (horse.stats.distancePerformance.wins /
          horse.stats.distancePerformance.starts) *
          100
      );
      var distancePerformancePlacePercentage = Math.round(
        ((horse.stats.distancePerformance.wins +
          horse.stats.distancePerformance.seconds) /
          horse.stats.distancePerformance.starts) *
          100
      );

      var trackDistanceWinPercentage = Math.round(
        (horse.stats.trackAndDistancePerformance.wins /
          horse.stats.trackAndDistancePerformance.starts) *
          100
      );
      var trackDistancePlacePercentage = Math.round(
        ((horse.stats.trackAndDistancePerformance.wins +
          horse.stats.trackAndDistancePerformance.seconds) /
          horse.stats.trackAndDistancePerformance.starts) *
          100
      );
      ...
      prizeMoneyCell.innerHTML = Math.round(prizeMoney);
      ...
      var averagePrizeMoney = Math.round(
        prizeMoney / horse.stats.allPerformance.starts
      );
      ...
      previousForms.forEach((form) => {
        if (form.Finish <= 3) {
          ...
          if (barrier == form.Barrier) {
            ...
            barrier == form.Barrier + 1 ||
            barrier == form.Barrier - 1
          ) {
            pointBreakdownComment += "-Barrier +-1 points +2<br/>";
            formBonus += 2;
          } else if (
            barrier == form.Barrier + 2 ||
            barrier == form.Barrier - 2
          ) {
          ...
          if (form.RacePrizeMoney > 5000000) {
            pointBreakdownComment +=
              "-RacePrizeMoney > 5,000,000 points +15<br/>";
            formBonus += 15;
          } else if (form.RacePrizeMoney > 3000000) {
            pointBreakdownComment +=
              "-RacePrizeMoney > 3,000,000 points +13<br/>";
            formBonus += 13;
          } else if (form.RacePrizeMoney > 1000000) {
            pointBreakdownComment +=
              "-RacePrizeMoney > 1,000,000 points +10<br/>";
            formBonus += 10;
        ...
        if (form.Finish <= 3) {
          var trackCondition = form.TrackCondition;
          ...
          if (trackCondition == raceCondition.charAt(0)) {
            conditionBonus = 5;
            pointBreakdownComment +=
          ...
          if (form.TimeRan != "N/A") {
            ...
            var barrierDiffirence = Number(barrier) - Number(form.Barrier);
            ...
            if (raceDistance == 1400) {
              if (
                trackCondition.toLowerCase() == "g" &&
                raceCondition.charAt(0).toLowerCase() == "h"
              ) {
                miliToWin += 400;
              } else if (
                trackCondition.toLowerCase() == "h" &&
                raceCondition.charAt(0).toLowerCase() == "g"
              ) {
                miliToWin -= 400;
              }
            }
          ...
          var formInfo = "";
          if (form.Distance == raceDistance) {
            formInfo += "-<b>Distance: " + form.Distance + "</b>";
          } else {
            formInfo += "-Distance: " + form.Distance;
          }
          if (trackCondition == raceCondition.charAt(0)) {
            formInfo += "<br/>-<b>Condition: " + trackCondition + "</b>";
          } else {
            formInfo += "<br/>-Condition: " + trackCondition;
          }
          formInfo +=
            "<br/>-Finish: " +
            finish +
      ...
      if (
        isNaN(trackPerformanceWinPercentage) ||
        trackPerformanceWinPercentage == 0 ||
        trackPerformancePlacePercentage < 50
      ) {
        trackBonus -= 10;
      } else {
      ...
      if (
        isNaN(trackPerformancePlacePercentage) ||
        trackPerformanceWinPercentage == 0 ||
        trackPerformancePlacePercentage < 50
      ) {
        distanceBonus = -10;
      ...
      if (
        isNaN(trackDistancePlacePercentage) ||
        trackDistanceWinPercentage == 0 ||
        trackDistancePlacePercentage < 50
      ) {
  ...
  averagePrizeArray.forEach(function (prize, prizeIndex) {
    availableHorses.forEach(function (horse, horseIndex) {
      ...
      if (averagePrizeMoney == prize) {
        ...
        console.log("total: " + Number(points + pointsForPrize));
        finalPoints = Number(points + pointsForPrize);
        pointsCellArray[horseIndex].innerHTML = finalPoints;
        pointBreakdownComentArray[horseIndex] +=
          "-Average Prize, points " + pointsForPrize + " <br/>";
        pointBreakdownComentArray[horseIndex] +=
          "-Total points =  " + finalPoints + " <br/>";
        console.log("comment = " + pointBreakdownComentArray[horseIndex]);
        commentCellArray[horseIndex].innerHTML =
          pointBreakdownComentArray[horseIndex];
      }
    });
  });
